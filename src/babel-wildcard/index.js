    ...
    return {
        visitor: {
            ImportDeclaration(path, state) {
                ...
                const alias = state.opts.alias;

                /**
                 * THIS IS THE REPLACEMENT, 
                 */
                //var src = Object.keys(alias).reduce((res, cur) => path.node.source.value === cur ? alias[cur] : res, path.node.source.value) 
                //var src = path.node.source.value !== "__SOURCES__" ? path.node.source.value : alias;

                //console.log(Object.keys(alias))
                var src = Object.keys(alias).reduce((res, cur) => (
                ...
                if (addWildcard) {
                    ...
                    try {
                        ...
                        for (var i = 0; i < r.length; i++) {
                            //console.log(dir)
                            //if (dir.indexOf("node_modules") !== -1) continue;
